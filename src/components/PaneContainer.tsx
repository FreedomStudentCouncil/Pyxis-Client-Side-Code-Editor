    ...
    >
      <TabBar
        ...
        onTabClose={tabId => {
          ...
          const actualPaneIndex = flatPanes.findIndex(p => p.id === pane.id);
          if (actualPaneIndex >= 0) {
            setTabsForPane(allPanes, setEditors, actualPaneIndex, pane.tabs.filter(t => t.id !== tabId));
            if (pane.activeTabId === tabId) {
              const newActive = pane.tabs.filter(t => t.id !== tabId);
              setActiveTabIdForPane(allPanes, setEditors, actualPaneIndex, newActive.length > 0 ? newActive[0].id : '');
            }
          }
        }}
        isBottomPanelVisible={isBottomPanelVisible}
        ...
        onMoveTabToPane={(tabId, targetPaneId) => {
          ...
          if (sourcePaneIndex >= 0 && targetPaneIndex >= 0) {
            // 移動元から削除
            setTabsForPane(allPanes, setEditors, sourcePaneIndex, pane.tabs.filter(t => t.id !== tabId));
            
            // 移動先に追加
            setTabsForPane(allPanes, setEditors, targetPaneIndex, (prevTabs) => [...prevTabs, sourceTab]);
            ...
            if (pane.activeTabId === tabId) {
              ...
              setActiveTabIdForPane(allPanes, setEditors, sourcePaneIndex, remainingTabs.length > 0 ? remainingTabs[0].id : '');
            }
          }
        }}
